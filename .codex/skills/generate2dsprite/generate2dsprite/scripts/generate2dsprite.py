...
def cmd_process(args: argparse.Namespace) -> None:
    ...
    if args.aspect_ratio_target is not None and args.aspect_ratio_target <= 0:
        raise ValueError("--aspect-ratio-target must be greater than 0.")
    if args.aspect_ratio_tolerance < 0:
        raise ValueError("--aspect-ratio-tolerance must be >= 0.")
    preserve_native_scale = not args.normalize_scale
    ...
    if has_custom_grid or args.mode in GRID_SHAPES:
        ...
        if preserve_native_scale and args.cell_width is None and args.cell_height is None and args.cell_size is None:
        ...
        aspect_ratio_target = args.aspect_ratio_target
        valid_ratios: list[float] = []
        for info in frame_qc:
            output_size = info.get("output_size") or [0, 0]
            width = int(output_size[0])
            height = int(output_size[1])
            if width > 0 and height > 0:
                valid_ratios.append(width / height)
        if aspect_ratio_target is None and valid_ratios:
            aspect_ratio_target = float(median(valid_ratios))

        aspect_ratio_drift_frames: list[list[int]] = []
        for info in frame_qc:
            output_size = info.get("output_size") or [0, 0]
            width = int(output_size[0])
            height = int(output_size[1])
            if width > 0 and height > 0:
                ratio = width / height
                info["output_aspect_ratio"] = ratio
                if aspect_ratio_target and aspect_ratio_target > 0:
                    drift = abs(ratio - aspect_ratio_target) / aspect_ratio_target
                    info["aspect_ratio_drift"] = drift
                    if drift > args.aspect_ratio_tolerance:
                        aspect_ratio_drift_frames.append(list(info["grid"]))
                else:
                    info["aspect_ratio_drift"] = None
            else:
                info["output_aspect_ratio"] = None
                info["aspect_ratio_drift"] = None
        ...
        metadata["preserve_native_scale"] = preserve_native_scale
        ...
        metadata["aspect_ratio_target"] = aspect_ratio_target
        metadata["aspect_ratio_tolerance"] = args.aspect_ratio_tolerance
        ...
        metadata["aspect_ratio_drift_frames"] = aspect_ratio_drift_frames
        ...
        if args.reject_aspect_ratio_drift and aspect_ratio_drift_frames:
            raise ValueError(
                "Frames drift from target aspect ratio beyond tolerance: "
                f"{aspect_ratio_drift_frames} (target={aspect_ratio_target:.4f}, "
                f"tolerance={args.aspect_ratio_tolerance:.4f})"
            )
    else:
        ...
        if preserve_native_scale:
            ...
            cleaned.save(out_dir / "sheet-transparent.png")
            save_transparent_gif([cleaned], out_dir / "animation.gif", args.duration)
        else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    process_parser.add_argument(
        "--normalize-scale",
        action="store_true",
        help="Enable frame resizing and scale normalization. Default behavior preserves native frame scale.",
    )
    ...
    process_parser.add_argument("--aspect-ratio-target", type=float)
    process_parser.add_argument("--aspect-ratio-tolerance", type=float, default=0.03)
    process_parser.add_argument("--reject-aspect-ratio-drift", action="store_true")
